...
To support our data analysis the tweets were stored in a database. We downloaded many GBs of text files containing the tweets. Parsing through these with a Python script was inefficient. To speed up our analysis a script read in each text file, extracted fields of interest (e.g.., tweet text, created at, author name) and stored that information in a simple database table, as shown below with data fields removed.
...
Excel files were created containing all the tweets for a query. These files were generated using a Python script which queried our tweet database. If datasets were over 100,000 rows then the tweets were split into multiple excel files. From these files, we identified a list of relevant conversation IDs for each dataset.

Our automated analysis created several outputs from the relevant conversation IDs. A script extracted the tweets for each conversation from the database. These tweets were written to (a) an Excel file ordered by conversation ID and creation date and a text representation of reply chains.
